...
/** Settings - puppy sounds */
function settingpuppySounds() {
    let state = elEnablepuppySounds.checked;
    console.log(`enablepuppySounds set to ${state}`);
    toast("Settings", `puppy sounds are now ${state ? 'enabled' : 'disabled'}`,
    '', false, true);

    // localStorage
    settings.enablepuppySounds = state;
    saveSettings();
}
...
document.addEventListener('keyup', event => {
    ...
    // puppy click
    if(key === settings.keybinds['key_puppy']) {
        puppyClick(false, false, 1);
        keypuppyFiring = false;
        holdStop();
...
const keyCodesCode = {
    ...
    2: () => {
        ...
        // mouseConfetti([24,24], ccpuppy, 600);
    },
...
function eggUp() {
    ...
    mouseConfetti([1, easterEgg === 101 ? 2 : Math.floor(easterEgg/5)], ccpuppy);
    ...
    function hash() {
        ...
        if(isDebug()) {
            ...
            window.updateValues = event => {
                // puppys
                let cc = parseInt(setpuppysEl.value);
                player.puppys          = 0;
                player.lifetime.puppys = 0;
                player.prestige.puppys = 0;
                earnpuppys(cc);
                // Cash
                if(setCashEl.value !== '') {
                ...
                // Golden puppys
                if(setGoldenpuppysEl.value !== '') {
                    let gcc = parseInt(setGoldenpuppysEl.value);
                    player.golden_puppys = gcc
                    updateGC();
                    updateCharlesShop();
                }
            ...
            `<div style="">
                <div class="footer_bottom brown_darker_color">
                    ...
                    <table>
                        <tr>
                            <td>
                                <label for="setpuppy">puppys:</label>
                            </td>
                            <td>
                                <input id="setpuppy" class="dev_input" type="number" value="500000">
                            </td>
                            <td id="setpuppyRounded" class="secondary_text"></td>
                        </tr>

                        <tr>
                            <td>
                                <label for="setGoldenpuppy">Golden puppys:</label>
                            </td>
                            <td>
                                <input id="setGoldenpuppy" class="dev_input" type="number">
                            </td>
                        </tr>
            ...
            window.setpuppysEl       = dom("setpuppy");
            window.setGoldenpuppysEl = dom("setGoldenpuppy");
            window.setCashEl          = dom("setCash");
            window.elSetpuppyRounded = dom('setpuppyRounded')
            ...
            setpuppysEl.addEventListener('input', () => {
                if(setpuppysEl.value === '') return;
                elSetpuppyRounded.innerText = `(${DisplayRounded(setpuppysEl.value)})`;
            });
        }
...
(() => {
    ...
    if(default_player.data_version > player.data_version || isDebug() && !player.flags.debug_dont_autoupdate) {
        try {
            ...
            if(!isDebug())
             toast(
                'Update Beta 1.17.5',
                '- New Bill cosmetic<br/>- New achievement & achievement icons<br/>- Achievement unlock condition changes & bug fixes<br/><br/><a href="https://www.notkal.com/posts/puppy-clicker-changelog/#dev-beta-1_17_5" target="_blank">Full changelog</a>',
                undefined, true,);
    ...
    //     // Convert to seconds
         ...
         let difference = Math.round(Date.now() / 1000) - player.time_last_saved;
         let earned = difference * player.cps;

         toast('Offline Earnings', `${difference} seconds\n${earned} puppys`, 'purple', true);
    ...
    if(player.flags['cookies_accepted'] !== true) {
        let cookieToast = toast(
            'Cookies & Privacy',
            'By clicking accept you agree to the usage of cookies to save your progress. If you choose not to click accept your progress will not be saved. <a href="https://www.yourwesite.com/about/#privacy" target="_blank">Privacy Policy</a>',
    ...
    if(player.flags.tutorial0 !== true) {
        toast("Welcome to puppy Clicker!",
        "Click the puppy to farm. Spend your puppys on hiring/upgrading new workers. Eventually you will be able to buy them better tools to work with. Good luck!",
        "", true);
        player.flags.tutorial0 = true;
    }
    // Update notice
    if(player.flags['v1.2_changelog'] !== true) {
         toast("Version 1.2 changelog", "puppy Clicker 1.0 has been released. Read the changelog here:", "", true);
         player.flags['v1.2_changelog'] = true;
     }
